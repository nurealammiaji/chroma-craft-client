        ...
        <nav className="relative">
            <div style={{ background: `url(${shape1}) center no-repeat` }} className="fixed z-10 max-w-screen-xl font-semibold text-pink-800 border rounded-full shadow-md px-auto navbar border-primary top-1">
                ...
                <div className="navbar-end [&>*]:ml-1 hover:[&>*]:text-purple-700 text-base">
                    ...
                    {
                        <div className="mr-4 drawer-content tooltip" data-tip="Cart">
                    ...
                    }
                    {
                        (user && userData) ?
                            <>
                                <div className="mr-2 avatar tooltip" data-tip={`${userData?.name}`}>
                                    <div className="w-4 rounded-full ring ring-neutral ring-offset-base-100 ring-offset-2">
                    ...
                    <ul className="min-h-full p-4 menu w-80 bg-base-200 text-base-content">
                        ...
                        <form onSubmit={handleSubmit2(handleSearch)} className="my-0 mb-4">
                            ...
                            {errors2.searchText?.type === 'required' && <label className="mt-3 label">
                                <span className="text-warning">Please type any keyword !!</span>
                            </label>}
                            {
                                (searchResult) &&
                                <label className="mt-3 label">
                                    {
                                        (searchResult.length === 0) ?
                                            <span className="font-semibold text-error">Sorry, Not Found !!</span> :
                                            <span className="font-semibold text-success">Found {searchResult?.length} {searchResult.length === 1 ? "Result" : "Results"} !!</span>
                                    }
                                </label>
                            }
                        </form>
                        {
                            (searchResult) &&
                            searchResult.map((item, index) => <li className="my-1 border rounded-full border-neutral" key={item._id}><NavLink to={`/classes/${item?._id}`}>{index + 1}. {item?.title}</NavLink></li>)
                        }
                    </ul>
                </div>
            </div>
